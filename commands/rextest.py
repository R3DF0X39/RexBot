...
class test:
    ...
    async def test(self , ctx, language, *, input : str):#This is like the command input, so for this one you have to say "say (input)" and it will output the input
        ...
        try:
            ...
            #Building embed
            build=discord.Embed(colour=discord.Colour(value=0x008fff))
            build.set_author(name="Rex Tester", icon_url="https://i.imgur.com/n9q9nBU.png")
            build.set_footer(text="Command issued by {}".format(author))
            buildprint = await self.bot.edit_message(start, embed=build)
            await self.bot.send_typing(ctx.message.channel)
            time.sleep(0.420)
            build2=discord.Embed(colour=discord.Colour(value=0x008fff))
            build2.set_author(name="Rex Tester", icon_url="https://i.imgur.com/n9q9nBU.png")
            build2.add_field(name="Warnings", value="Not started".format(warns), inline=False)
            build2.set_footer(text="Command issued by {}".format(author))
            buildprint2 = await self.bot.edit_message(buildprint, embed=build2)
            await self.bot.send_typing(ctx.message.channel)
            time.sleep(0.420)
            build3=discord.Embed(colour=discord.Colour(value=0x008fff))
            build3.set_author(name="Rex Tester", icon_url="https://i.imgur.com/n9q9nBU.png")
            build3.add_field(name="Warnings", value="Not started", inline=False)
            build3.add_field(name="Errors", value="Not started", inline=False)
            build3.set_footer(text="Command issued by {}".format(author))
            buildprint3 = await self.bot.edit_message(buildprint2, embed=build3)
            await self.bot.send_typing(ctx.message.channel)
            time.sleep(0.420)
            build4=discord.Embed(colour=discord.Colour(value=0x008fff))
            build4.set_author(name="Rex Tester", icon_url="https://i.imgur.com/n9q9nBU.png")
            build4.add_field(name="Warnings", value="Not started", inline=False)
            build4.add_field(name="Errors", value="Not started", inline=False)
            build4.add_field(name="Stats", value="Not started", inline=False)
            build4.set_footer(text="Command issued by {}".format(author))
            buildprint4 = await self.bot.edit_message(buildprint3, embed=build4)
            await self.bot.send_typing(ctx.message.channel)
            time.sleep(0.420)
            ...
            buildprint5 = await self.bot.edit_message(buildprint4, embed=build5)
            await self.bot.send_typing(ctx.message.channel)
